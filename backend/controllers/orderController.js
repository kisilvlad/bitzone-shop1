...
const createOrder = asyncHandler(async (req, res) => {
  ...
  for (const cartItem of cartItems) {
    ...
    // ================== ПОЧАТОК ВИПРАВЛЕННЯ v4 ==================
    // Базуючись на документації (create-order-item) та логах,
    // API очікує прості структури з 2-х ключів кожна.
    const payload = {
      ...
      // Гіпотеза v4: Потрібні ключі: type, value
      discount: {
        type: 'percentage', // 'percentage' або 'value' (з логу v1)
        value: 0,
      },
      
      // Гіпотеза v4: Потрібні ключі: value, unit
      // (з логу v2 ми знаємо, що 1 з 2 ключів був вірний, ймовірно 'unit')
      warranty: {
        value: 0,
        unit: 'month', // 'day', 'month', 'year'
      },
    };
    // =================== КІНЕЦЬ ВИПРАВЛЕННЯ v4 ===================
